<?php

use function PHPSTORM_META\map;

$title=[
    "ar"=>"محمد أياو | مصمم مواقع",
    "en"=>"Mohamed Ayaou | Web/Graphic Designer"
];
$content = [
    "ar" => [
        "hero" => [
            "مصمم\مطور مواقع الويب",
            "اسمي هو محمد أياو وانا مصمم جرافيك ومطور مواقع إنترنت من مدينة تزنيت بالمغرب, ويمكنك تصفح أعمالي أو التواصل معي مباشرة ",
            "تواصل",
            "أعمالي"
        ],
        "featuers" => [
            "ما المميز؟",
            [
                "one.png",
                "تصميم متوافق",
                "موقع يمكنه التأقلم مع جميع القياسات والشاشات والظهور بمظهري يناسب كل الحالات"
            ],
            [
                "one.png",
                "تصميم متوافق",
                "موقع يمكنه التأقلم مع جميع القياسات والشاشات والظهور بمظهري يناسب كل الحالات"
            ],
            [
                "one.png",
                "تصميم متوافق",
                "موقع يمكنه التأقلم مع جميع القياسات والشاشات والظهور بمظهري يناسب كل الحالات"
            ],
            [
                "one.png",
                "تصميم متوافق",
                "موقع يمكنه التأقلم مع جميع القياسات والشاشات والظهور بمظهري يناسب كل الحالات"
            ],
            [
                "one.png",
                "تصميم متوافق",
                "موقع يمكنه التأقلم مع جميع القياسات والشاشات والظهور بمظهري يناسب كل الحالات"
            ],
            [
                "one.png",
                "تصميم متوافق",
                "موقع يمكنه التأقلم مع جميع القياسات والشاشات والظهور بمظهري يناسب كل الحالات"
            ],
            [
                "one.png",
                "تصميم متوافق",
                "موقع يمكنه التأقلم مع جميع القياسات والشاشات والظهور بمظهري يناسب كل الحالات"
            ],
            [
                "one.png",
                "تصميم متوافق",
                "موقع يمكنه التأقلم مع جميع القياسات والشاشات والظهور بمظهري يناسب كل الحالات"
            ],
            [
                "one.png",
                "تصميم متوافق",
                "موقع يمكنه التأقلم مع جميع القياسات والشاشات والظهور بمظهري يناسب كل الحالات"
            ],
        ],
        "portfolio" => [
            "أعمالي",
            [
                "one.png",
                "نبذة عن المشروع",
                "موقع 'جليدي' هو متجر إلكتروني متخصص في بيع الأجهزة الالكترونية وإكسسوارات الكمبيوتر",
                "زيارة/معاينة"
            ],
            [
                "one.png",
                "نبذة عن المشروع",
                "موقع 'جليدي' هو متجر إلكتروني متخصص في بيع الأجهزة الالكترونية وإكسسوارات الكمبيوتر",
                "زيارة/معاينة"
            ],
            [
                "one.png",
                "نبذة عن المشروع",
                "موقع 'جليدي' هو متجر إلكتروني متخصص في بيع الأجهزة الالكترونية وإكسسوارات الكمبيوتر",
                "زيارة/معاينة"
            ],
            [
                "one.png",
                "نبذة عن المشروع",
                "موقع 'جليدي' هو متجر إلكتروني متخصص في بيع الأجهزة الالكترونية وإكسسوارات الكمبيوتر",
                "زيارة/معاينة"
            ]
        ],
        "services" => [
            "الخدمات",
            [
                "one.png",
                "التصميم",
                "تصميم وهندسة بنية الموقع وكامل الهوية البصرية للمقاولة",
                "100$"
            ],
            [
                "one.png",
                "التطوير",
                "تحويل تصميم جرافيك إلى موقع تفاعلي أو إعادة برمجة موقع قديم",
                "100$"
            ],
            [
                "one.png",
                "التأمين",
                "تأمين الموقع من أكثر من 12 هجوم إلكترونيا",
                "100$"
            ],
        ],
        "rates" => [
            "آراء العملاء"
        ],
        "contact" => [
            "التواصل",
            [
                "الإسم",
                "الهاتف(إختياري)",
                "البريد الإلكتروني",
                [
                    "التصميم",
                    "التطوير",
                    "التأمين",
                    "خدمة مخصصة"
                ],
                "الرسالة",
                "ارسال"
            ]
        ],
        "about" => [
            "حولي",
            "اسمي هو محمد أياو وانا مصمم جرافيك ومطور مواقع إنترنت من مدينة تزنيت بالمغرب, ويمكنك تصفح أعمالي أو التواصل معي مباشرة, اسمي هو محمد أياو وانا مصمم جرافيك ومطور مواقع إنترنت من مدينة تزنيت بالمغرب, ويمكنك تصفح أعمالي أو التواصل معي مباشرة ",
            "رؤية CV"
        ],
        "blog" => "",
        "policies" => [
            "سياسات الدفع والتعاقد",
            [
                "تتوفر عدة طرق للدفع على حسب رغبة العميل, وكل الدفعات يتم الاتفاق عليها مسبقا في العقد قبل الشروع في العمل, العقد الالكتروني معتبر قانونيا وأي انتهاكات لحقوق أحد الطرفين فإن العقد يردها ",
                "يمكن الانسحاب من العقد متى شاء العميل مع استرداده لآخر دفعة",
                "يسترد العميل 50% من المبلغ بعدانتهاء العمل في حالة لم يرضى بالنتيجة",
                "طرق الدفع المقبولة:"
            ]
        ]
    ],
    "en" => [
        "hero" => [
            "Web/Graphic Designer",
            "My name is Mohamed Ayaou and I am a self-thought web and graphic designer from Morroco, You can take a look on my porfolio or contact me directly.",
            "Contact",
            "Portfolio"
        ],
        "featuers" => [
            "What is special?",
            [
                "one.png",
                "UI/UX",
                "A clear and modern designe with the academic ui/ux's rules."
            ],
            [
                "one.png",
                "UI/UX",
                "A clear and modern designe with the academic ui/ux's rules."
            ],
            [
                "one.png",
                "UI/UX",
                "A clear and modern designe with the academic ui/ux's rules."
            ],
            [
                "one.png",
                "UI/UX",
                "A clear and modern designe with the academic ui/ux's rules."
            ],
            [
                "one.png",
                "UI/UX",
                "A clear and modern designe with the academic ui/ux's rules."
            ],
            [
                "one.png",
                "UI/UX",
                "A clear and modern designe with the academic ui/ux's rules."
            ],
            [
                "one.png",
                "UI/UX",
                "A clear and modern designe with the academic ui/ux's rules."
            ],
            [
                "one.png",
                "UI/UX",
                "A clear and modern designe with the academic ui/ux's rules."
            ],
            [
                "one.png",
                "UI/UX",
                "A clear and modern designe with the academic ui/ux's rules."
            ],
        ],
        "portfolio" => [
            "My Portfolio",
            [
                "one.png",
                "About The Project",
                "'JELLEED' website is an ecommerce site for selling elecctronincs and accessories.",
                "Visit/Preview"
            ],
            [
                "one.png",
                "About The Project",
                "'JELLEED' website is an ecommerce site for selling elecctronincs and accessories.",
                "Visit/Preview"
            ],
            [
                "one.png",
                "About The Project",
                "'JELLEED' website is an ecommerce site for selling elecctronincs and accessories.",
                "Visit/Preview"
            ],
            [
                "one.png",
                "About The Project",
                "'JELLEED' website is an ecommerce site for selling elecctronincs and accessories.",
                "Visit/Preview"
            ]
        ],
        "services" => [
            "Services",
            [
                "one.png",
                "Designing",
                "Desinging and palnning the sturcture of the site with all of it visual identity.",
                "100$"
            ],
            [
                "one.png",
                "Developing",
                "Converting the graphical design to a real interactive website.",
                "100$"
            ],
            [
                "one.png",
                "securing",
                "Securing your website from more than 12+ web attacks",
                "100$"
            ],
        ],
        "rates" => [
            "Clients' Rates"
        ],
        "contact" => [
            "Contact",
            [
                "Name",
                "Phone(Optional)",
                "Email",
                [
                    "Designing",
                    "Developing",
                    "Securing",
                    "Custom"
                ],
                "The Message",
                "Send"
            ]
        ],
        "about" => [
            "About Me",
            "My name is Mohamed Ayaou and I am a self-thought web and graphic designer from Morroco, You can take a look on my porfolio or contact me directly.My name is Mohamed Ayaou and I am a self-thought web and graphic designer from Morroco, You can take a look on my porfolio or contact me directly.",
            "MY CV"
        ],
        "blog" => "",
        "policies" => [
            "Payment & Contarcting Policies",
            [
                "There are several payment methods available according to the customer’s desire, and all payments are agreed upon in advance in the contract before starting work. The electronic contract is considered legal and any violations of the rights of one of the parties are covered by the contract. ",
                "It is possible to withdraw from the contract whenever the customer wants, with his refund of the last payment",
                "The customer gets 50% of the amount back after the completion of the work in case he is not satisfied with the result",
                "The available payment methods:"
            ]
        ]
    ]
];

